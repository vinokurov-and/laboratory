...
type DisciplinesType = Record<
  string,
  { label: string; curPath: string; labs: { file: string; url: string } }
>[];

export const DISCIPLINES = disciplinesFolders.reduce(
  (prevValue, curVal, index) => {
    ...
    return { ...prevValue, [curVal]: { label: LABELS[curVal], labs: lab } };
  },
  {}
) as DisciplinesType;
